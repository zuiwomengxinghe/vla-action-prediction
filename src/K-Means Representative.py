...
def direct_coreset_selection(
    ...
    """
    使用 K-Means 聚类选择 coreset（基于图像 embedding）
    对于每个聚类，选择离聚类中心最近的样本作为代表，共选择 k_coreset 个。
    """
    ...
    if cache is not None:
        ...
        if cached_final_df is not None and cached_final_image is not None and cached_final_text is not None:
            print("  [Cache] Using cached final coreset selection results (K-Means).")
            print("  [Warning] If you want to re-run with new parameters, delete the cache directory.")
    ...
    # ---- K-Means 聚类选择 (基于图像 embedding) ----
    print(f"Running K-Means clustering on image embeddings (n_clusters={k_coreset})...")
    image_np = image_norm.cpu().numpy()

    if total_frames <= k_coreset:
        selected_indices = list(range(total_frames))
        print(f"Total frames ({total_frames}) <= k_coreset ({k_coreset}), using all frames.")
    else:
        # 启用详细日志
        kmeans = KMeans(n_clusters=k_coreset, random_state=42, n_init=10, verbose=1)
        print("Starting K-Means (verbose=1)...")
        labels = kmeans.fit_predict(image_np)
        centers = kmeans.cluster_centers_
        print("K-Means finished. Selecting closest sample per cluster...")

        selected_indices = []
        total_clusters = k_coreset
        for cluster_id in range(total_clusters):
            cluster_mask = (labels == cluster_id)
            cluster_indices = np.where(cluster_mask)[0]
            if len(cluster_indices) == 0:
                print(f"Warning: cluster {cluster_id} has no samples, fallback to random.")
                remaining = set(range(total_frames)) - set(selected_indices)
                if remaining:
                    selected_indices.append(np.random.choice(list(remaining)))
                continue
            # 计算该聚类内所有样本到聚类中心的欧氏距离
            dists = np.linalg.norm(image_np[cluster_indices] - centers[cluster_id], axis=1)
            closest_idx_in_cluster = cluster_indices[np.argmin(dists)]
            selected_indices.append(closest_idx_in_cluster)

            # 每100个聚类打印一次进度
            if (cluster_id + 1) % 100 == 0 or cluster_id == total_clusters - 1:
                print(f"  Selected for {cluster_id+1}/{total_clusters} clusters")

        # 确保正好选了 k_coreset 个
        if len(selected_indices) < k_coreset:
            missing = k_coreset - len(selected_indices)
            remaining = set(range(total_frames)) - set(selected_indices)
            if len(remaining) >= missing:
                selected_indices.extend(np.random.choice(list(remaining), missing, replace=False))
            else:
...
def parse_args():
    ...
    parser.add_argument("--k-coreset", type=int, default=2000, help="Final coreset size after K-Means selection")
...
def main():
    ...
    print("\n" + "=" * 60)
    print("DIRECT K-MEANS CORESET SELECTION (on image embeddings)")
    print("=" * 60)
